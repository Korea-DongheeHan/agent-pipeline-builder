...
6. **루프 상한 엄수** — 피드백 루프(loop / 뒤로 goto)는 `max` 를 초과할 수
   ...
   노드 = 그 노드로 위임).
6-1. **게이트 노드(`gate: true`)** — 에이전트를 띄우지 않는다. 선행 산출물
   (예: analyst 의 `## 확정 필요 질문`)을 근거로 **그 자리에서
   AskUserQuestion** 을 수행하고, 확정 결과를 후속 노드 prompt 에 주입한
   뒤 진행한다. 사용자가 답하기 전에는 후속 노드를 실행하지 않는다.
7. **산출물 보존** — 노드별 결과 전문은
